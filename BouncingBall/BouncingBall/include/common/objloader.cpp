...
bool loadOBJ(
	...
	if (!file.is_open()) {
		cout << "> Impossible to open the file \"" << path << "\", aborting...";
		return false;
	}
	...
	while (getline(file, line))
	{
		...
		if (lineHeader == "v") {
			...
			linestream >> delim >> normalIndex[0];

			linestream >> vertexIndex[1] >> delim >> uvIndex[1];
			if (linestream.fail()) linestream.clear();
			linestream >> delim >> normalIndex[1];

			linestream >> vertexIndex[2] >> delim >> uvIndex[2];
			if (linestream.fail()) linestream.clear();
			linestream >> delim >> normalIndex[2];
